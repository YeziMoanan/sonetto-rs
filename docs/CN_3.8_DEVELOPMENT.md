...
- 服务端自动化使用内存 SQLite migrations 与本地 TCP socket 覆盖 SDK mail/visitor/verify/autologin、verify/autologin 失败、欢迎邮件、TCP `on_login`、`handle_client` 错误阶段、过期体力物品/月卡登录初始化、签到错误传播和连接收尾日志；这是服务端隔离测试证据，不代表真实 APK 已连接本地服务或完成登录。
